...
public class MainMenuView : ComputerView {
    ...
    private readonly Dictionary<IComputerViewEntry, PluginInfo> _pluginInfos = [];
    ...
    public void ShowViewEntries(List<IComputerViewEntry> viewEntries) {
        ...
        foreach (IComputerViewEntry viewEntry in viewEntries) {
            ...
            PluginInfo pluginInfo = Chainloader.PluginInfos.Values.FirstOrDefault(x => x.Instance.GetType().Assembly == assembly);
            if (pluginInfo != null)
                _pluginInfos.Add(viewEntry, pluginInfo);
        }
    ...
    private void FilterViewEntries() {
        ...
        foreach (IComputerViewEntry viewEntry in _viewEntries) {
            if (!_pluginInfos.TryGetValue(viewEntry, out PluginInfo info))
                continue;
